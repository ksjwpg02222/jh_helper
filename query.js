...
const spreadSheetsId = '1-E6eQcZe-xf0IYXusb5Cb0MJ4BCYViAqy5oRN31VMiM'
const sheetId = '1687369622'
        ...
        try {
            if (target) {
                const exampleEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: '本周應繳金額', value: target[2] || '無資料', inline: true },
                        { name: '是否已繳納', value: target[3] || '無資料', inline: true },
                        { name: '\u200B', value: '\u200B' }
                    )
                    .addFields(
                        { name: 'CTA出席次數', value: target[1] || '無資料', inline: true },
                        { name: '可退稅金額', value: target[4] || '無資料', inline: true },
                        { name: '補裝箱編號', value: target[5] || '無資料', inline: true }
                    )
                    .setTimestamp()
